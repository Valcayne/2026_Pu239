...
int main(int argc, char **argv) {
  if (argc != 8 && argc != 9 && argc != 10) {
    cout << " ########## Input has to be like: CreateCalibFiles01 [InpCalibDir] "
  ...
  int RefRun = 0;
  TGraph *gr1 = GetGainMonitoring(DetName, DetID, GainShiftsDir, RefRun, RunDescriptor);
  double MinRun_GS = 0, MaxRun_GS = 0, GS_min = 0, GS_max;
  gr1->GetPoint(0, MinRun_GS, GS_min);
  gr1->GetPoint(gr1->GetN() - 1, MaxRun_GS, GS_max);

  // TGraph* graph = new TGraph(10);  // assume graph has 10 points
  ...
  CalibrationManager *theRefRunCal = new CalibrationManager(InpCalibDir, "TEST", RefCalibRunNumber);
  double p0_ref = 0, p1_ref = 0, p2_ref = 0, p3_ref = 0, p4_ref = 0, p5_ref = 0, p6_ref = 0;
  theRefRunCal->GetEneCalibParams(DetName, DetID, p0_ref, p1_ref, p2_ref, p3_ref, p4_ref, p5_ref, p6_ref);
  //------------------------------------------------------------------
  ...
  while (in1 >> inCalibFname) {
    ...
    if (thisRun % 100 == 0) {
      std::cout << "Doing run " << thisRun << std::endl;
    }

    if (thisRun >= RunMin && thisRun <= RunMax) {
      CalibrationManager *theCalMan = new CalibrationManager(InpCalibDir, "Test", thisRun, 0);
      double GainShift = -1;

      if (thisRun < MinRun_GS) {
        GainShift = GS_min;
        cout << " Gain shift for run " << thisRun << " is " << GainShift << " take min" << endl;

      } else if (thisRun > MaxRun_GS) {
        GainShift = GS_max;
        cout << " Gain shift for run " << thisRun << " is " << GainShift << " take max" << endl;
        ...
        GainShift = gr1->Eval(thisRun);
        cout << " Gain shift for run " << thisRun << " is " << GainShift << " interpolate " << endl;
      }
      double p0 = p0_ref;
...
void ChangeRefRun(TGraph *gr1, int newRefRun) {
  ...
  if (newRefRun < MinRun_GS || newRefRun > MaxRun_GS) {
    cout << "MinRun_GS " << MinRun_GS << " newRefRun " << newRefRun << " MaxRun_GS " << MaxRun_GS << endl;
    cout << " ############# ERROR in " << __FILE__ << ", line " << __LINE__ << " #############" << endl;
    exit(1);
  }
