...
// #define HISTOS2D_DIR \
//   "/eos/home-v/valcayne/nTOFDataProcessing/2026_Sm/2DHistos/v01/"

struct GainMonInfo {
  string detName = "none";
  int DetID = 1, PulseType = 1, rebin = 1, npShift = 200,
      HalfNPToFitParabolic = 25;
  double EnMin = 1., EnMax = 5.e3, ampMin = 0, ampMax = 1.e5, ShiftMin = 0.8,
         ShiftMax = 1.2, MinNCounts = 1.e4, ExtraShift = 1.0;
  bool norm = true;
};

//===============================================================================
TH1D* GetEdepSpectrum(int run, string detName, int DetID, int PulseType,
                      double EnMin, double EnMax, double& npulses,
                      double& nprotons);
double GetGainChange(int run1, int run2, string detName, int DetID,
                     int PulseType, int rebin, double EnMin, double EnMax,
                     double ampMin, double ampMax, bool PLOT = true,
                     double ShiftMin = 0.8, double ShiftMax = 1.2,
                     int npShift = 200, bool NORM = true,
                     double MinNCounts = 1.e4, int HalfNPToFitParabolic = 25);
double GetGainChange(int run1, int run2, string detName, int DetID,
                     string inpfile);
std::vector<GainMonInfo> ReadGainMonInput(string fname);
TGraph* GetGainMonitoring(const char* DetName, int DetID, const char* outdir,
                          int& FirstRefRun, const char* RunDescriptor = 0,
                          bool Recalculate = true);
int ReCalculateGainShifts(int nRuns, double* RunNumber, double* RefRun,
                          double* GainShift, bool Recalculate = true);
...
double GetGainChange(int run1, int run2, string detName, int DetID,
  ...
  double GainChange = GetGainChange(
      run1, run2, theMonInfo->detName, theMonInfo->DetID, theMonInfo->PulseType,
      theMonInfo->rebin, theMonInfo->EnMin, theMonInfo->EnMax,
      theMonInfo->ampMin, theMonInfo->ampMax, true, theMonInfo->ShiftMin,
      theMonInfo->ShiftMax, theMonInfo->npShift, theMonInfo->norm,
      theMonInfo->MinNCounts, theMonInfo->HalfNPToFitParabolic);
...
double GetGainChange(int run1, int run2, string detName, int DetID,
                     int PulseType, int rebin, double EnMin, double EnMax,
                     double ampMin, double ampMax, bool PLOT, double ShiftMin,
                     double ShiftMax, int npShift, bool NORM, double MinNCounts,
                     int HalfNPToFitParabolic) {
  cout << detName << "  " << DetID << "  " << PulseType << "  " << rebin << "  "
       << EnMin << "  " << EnMax << "  " << ampMin << "  " << ampMax << "  "
       << PLOT << "  " << ShiftMin << "  " << ShiftMax << "  " << npShift
       << "  " << NORM << "  " << MinNCounts << "  " << HalfNPToFitParabolic
       << endl;
  SetStyle01();
  // double MinNCounts=1.e4;
  double npulses1, npulses2;
  double nprotons1, nprotons2;
  TH1D* h1 = GetEdepSpectrum(run1, detName, DetID, PulseType, EnMin, EnMax,
                             npulses1, nprotons1);
  TH1D* h2 = GetEdepSpectrum(run2, detName, DetID, PulseType, EnMin, EnMax,
                             npulses2, nprotons2);
  if (h1 == 0 || h2 == 0) {
    return -1;
  }
  ...
  cout << " Number of counts in each histo --> " << NCounts1 << "  " << NCounts2
       << " , nbins = " << h1->GetNbinsX() / rebin << endl;
  if (NCounts1 < MinNCounts || NCounts2 < MinNCounts) {
    cout << " Number of counts smaller than " << MinNCounts << endl;
    return -2;
  }
  if (PulseType == 0) {
    h1->Scale(1. / npulses1);
  ...
  if (PLOT) {
    ...
    if (NORM) {
      h3->Scale(h2->Integral(h2->FindBin(ampMin), h2->FindBin(ampMax)) /
                h3->Integral(h3->FindBin(ampMin), h3->FindBin(ampMax)));
    }
    h1->SetLineColor(1);
    h2->SetLineColor(2);
    h3->SetLineColor(4);
...
TH1D* GetEdepSpectrum(int run, string detName, int DetID, int PulseType,
  ...
  sprintf(fname, "%s/Histos01_%d.root", outdir, run);
  sprintf(basehname, "Edep_%s", detName.c_str());

  TH1D* h1 = GetEdepFrom2DHisto(fname, basehname, DetID, PulseType, EnMin,
                                EnMax, npulses, nprotons);
  return h1;
}
...
std::vector<GainMonInfo> ReadGainMonInput(string fname) {
  ...
  while (getline(in, line)) {
    if (line[0] != '#') {
      ...
      iss >> theMonInfo.detName >> theMonInfo.DetID >> theMonInfo.PulseType >>
          theMonInfo.rebin >> theMonInfo.EnMin >> theMonInfo.EnMax;
      if (iss.good()) {
        ...
        while (iss >> word) {
          if (cont == 0) {
            theMonInfo.ShiftMin = stod(word);
          } else if (cont == 1) {
            theMonInfo.ShiftMax = stod(word);
          } else if (cont == 2) {
            theMonInfo.npShift = stoi(word);
          } else if (cont == 3) {
            theMonInfo.norm = stoi(word);
          } else if (cont == 4) {
            theMonInfo.MinNCounts = stod(word);
          } else if (cont == 5) {
            theMonInfo.HalfNPToFitParabolic = stoi(word);
          } else if (cont == 6) {
            theMonInfo.ExtraShift = stod(word);
          }
          cont++;
        }
...
TGraph* GetGainMonitoring(const char* DetName, int DetID, const char* outdir,
                          int& FirstRefRun, const char* RunDescriptor,
                          bool Recalculate) {
  char command[1000];
  sprintf(command, "ls %s/GShift_*.out > borrar.txt", outdir);
  if (RunDescriptor != 0) {
    sprintf(command, "ls %s/GShift_%s_*.out > borrar.txt", outdir,
            RunDescriptor);
  }
  ...
  while (in1 >> fname) {
    ...
    while (in2 >> detname >> detID >> Shift) {
      ...
      if (detID == DetID && string(detname) == string(DetName) && Shift > 0) {
        // cout << fname << endl;
        //  cout << " " << DetName << " " << DetID << " " << GainShift[np] << "
        //  "
        //<< endl;
...
// If ReCalculate==true then all the GainShift are transformed to be respect to
// the same RefRun
int ReCalculateGainShifts(int nRuns, double* RunNumber, double* RefRun,
                          double* GainShift, bool ReCalculate) {
  struct GainShiftByRun {
    double RunNumber, RefRun, GainShift;
    bool operator()(GainShiftByRun i, GainShiftByRun j) {
      return (i.RunNumber < j.RunNumber);
    }
  } theGSbyRun;
