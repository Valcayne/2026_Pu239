...
#include "../../GeneralFun/GeneralDefinitions.hh"

Monitor::Monitor(const char* ini_filename) {
  root_files_dir = outdirGainCorrected;
  std::cout << "ini_filename " << ini_filename << std::endl;
  /*root_files_dir =
   * "/afs/cern.ch/user/e/emendoza/eos/data/Er-2024/Histo2D-02";*/
  run_list_filename = RunListFname;
  run_type = ini_filename;

  // run_type = "EmilioRuns";
  const auto detectors = {
      Detector("SILI", {0, 1, 2, 3, 4}, {0.02, 1e5}, {1.8e4, 3e4}),
      // Detector("SILI", {0, 1, 2, 3, 4}, {0.02, 1.0}, {1.8e4, 3e4}),
      Detector("C6D6", {0, 1, 2, 3, 4}, {10, 1e5}, {0.150, 20})};
  ...
  // output_filename =
  //     dirnameMonitor + "/MonitoringRatio_Thermal" + run_type + ".root";
  output_filename = dirnameMonitor + "/MonitoringRatio_" + run_type + ".root
  // output_filename = dirnameMonitor + "/MonitoringRatio_" + run_type +
  // ".root";
  ...
  /*output_filename = "/eos/home-a/aperezde/Er_U/Monitoring.root";*/
}
...
void Monitor::Start() {
  ...
  for (const auto run : runs) {
    ...
    // std::cout << "Star run  " << run << std::endl;
