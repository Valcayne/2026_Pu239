...
struct CalibParams {
  ...
  double TOFD = -1, p0 = -1, p1 = -1, p2 = -1, p3 = -1, p4 = -1, p5 = -1,
         ...
         t1 = 0, w0 = 1, w1 = 0, w2 = 0, w3 = 0, w4 = 0, w5 = 0, w6 = 0;
};

//=======================================================================
class CalibrationManager {
 public:
  CalibrationManager(const char* calibdirname, const char* RunList, int run,
                     int verbose = 1);
  ~CalibrationManager();
  void CalibrationManagerWF(const char* WFFolder, const char* RunList, int run,
                            int verbose);
  double TakeTOF(Signal* P);     // ms
  double TakeTFlash(Signal* P);  // ms
  double TakeEnergy(Signal* P);  // MeV, use the amplitude

  double TakeTOFD(Signal* P);  // m
  double TakeWeight(Signal* P);
  bool IsInsideThresholdLimits(Signal* P);
  int WriteCalibFile(const char* calibdirname, int run);
  // double GetEneCalibParams(string DetName, int DetN, double& p0, double&
  // p1,
  //                          double& p2);
...
double CalibrationManager::TakeWeight(Signal* P) {
  int DetN = P->detn;
  string DetName = P->DetName;
  ...
  for (auto it = theCalibParams.begin(); it != theCalibParams.end(); ++it) {
    if (it->DetectorName == DetName && it->DetID == DetN) {
      ...
      Weight = it->w0 + it->w1 * ENERGY + it->w2 * ENERGY * ENERGY +
               it->w3 * ENERGY * ENERGY * ENERGY +
               it->w4 * ENERGY * ENERGY * ENERGY * ENERGY +
               it->w5 * ENERGY * ENERGY * ENERGY * ENERGY * ENERGY +
               it->w6 * ENERGY * ENERGY * ENERGY * ENERGY * ENERGY * ENERGY;
  ...
  // cout << "Ennergy" << ENERGY << " " << Weight << endl;
  return Weight;
}
...
double CalibrationManager::TakeEnergy(Signal* P) {
  ...
  for (auto it = theCalibParams.begin(); it != theCalibParams.end(); ++it) {
    ...
    // cout << "it->DetectorName " << it->DetectorName << " it->DetID" <<
    // it->DetID
    //    << endl;
  }
  if (!found) {
...
void CalibrationManager::CalibrationManagerWF(const char* WFFolder,
                                              const char* RunList, int run,
                                              int verbose) {
  string RunType = TakeRunTypeFunctions(RunList, run);

  string WFname = string(WFFolder) + "WF_" + RunType + ".in";
  ...
  ifstream in;
  in.open(WFname);
  cout << "WFname " << WFname << endl;
  if (!in.good()) {
    cout << WFname << "does not exist" << endl << endl;

    if (RunType.find("23") == 0) {
      RunType = "Default_23";
      WFname = string(WFFolder) + "WF_" + RunType + ".in";
      cout << "we use WF Default_23  " << WFname << endl;
    } else {
      RunType = "Default_24";
      WFname = string(WFFolder) + "WF_" + RunType + ".in";
      cout << "we use WF Default_24  " << WFname << endl;
    }
    in.open(WFname);
  ...
  while (in >> DetName) {
    if (DetName == '#') {
      ...
      for (auto it = theCalibParams.begin(); it != theCalibParams.end(); ++it) {
        if (it->DetectorName == DetName && it->DetID == DetN) {
          in >> it->w0 >> it->w1 >> it->w2 >> it->w3 >> it->w4 >> it->w5 >>
              it->w6;
        }
      }
    }
    cout << "Read" << DetName << " " << DetN << " " << endl;
  }
  in.close();
  cout << " WF information " << endl;
  for (auto it = theCalibParams.begin(); it != theCalibParams.end(); ++it) {
    cout << it->DetectorName << " " << it->DetID << " " << it->w0 << " "
         << it->w1 << " " << it->w2 << " " << it->w3 << " " << it->w4 << " "
         << it->w5 << " " << it->w6 << endl;
  }
...
CalibrationManager::CalibrationManager(const char* calibdirname,
                                       const char* RunList, int run,
                                       int verbose) {
  if (verbose > 0) {
