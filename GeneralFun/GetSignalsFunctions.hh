...
int GetSignals(string DetectorName, std::vector<double> DetectorDeadTime_ns,
  ...
  for (Long64_t i = EntryMin; i <= EntryMax; i++) {
    ...
    // cout << "DetectorName " << DetectorName << " SignalEne " << SignalEne
    //      << " SignalTOF " << SignalTOF << "P->amp" << theS->amp << "P->area"
    //      << theS->area << endl;
    SignalTFlash = TFlash;
    if (TFlash < 0) {
    ...
    if (CondType == 0 ||
      ...
      // cout << SignalTOF << " " << TOFPreviousSignal[theS->detn] << " "
      //      << SignalTOF - TOFPreviousSignal[theS->detn] << " "
      //      << DetectorDeadTime_ns[theS->detn] << endl;
      // cout << " detector number " << theS->detn << endl;
      // cout << " 3 " << DetectorDeadTime_ns.size() << endl;
