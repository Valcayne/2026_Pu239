...
// Macro to call pause easily, captures file and line automatically
#define PAUSE() PauseAt(__FILE__, __LINE__)
//--------------------------------------------------------------------------------------
void AttachStruct(Signal* theS, TTree* tr,
                  bool UseAreaInsteadOfAmplitude = false);
...
string TakeRunTypeFunctions(const char* fname, int RunNumber);
//---------------------------------------------------------------------------------------

//**********************************************************************************************************************
//**********************************************************************************************************************
// Function to pause and show file and line number
void PauseAt(const char* file, int line) {
  std::cout << "Paused at " << file << ", line " << line
            << ". Press ENTER to continue..." << std::endl;
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
}

void Pause(int lineNumber) {
  std::cout << "Paused at line " << lineNumber << ". Press ENTER to continue..."
            << std::endl;
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
}
...
void AttachStruct(Signal* P, TTree* tr, bool UseAreaInsteadOfAmplitude) {
  ...
  if (UseAreaInsteadOfAmplitude) {
    tr->SetBranchAddress("area", &(P->amp));
    tr->SetBranchAddress("amp", &(P->area));

    cout << endl
         << "###############" << endl
         << "Using area instead of amp" << endl
         << "###############" << endl
         << endl;
  } else {
...
int GetPulseType(Signal* theS) {
  if ((theS->PSpulse == DEDICATED_PULSES_ID_ROOTFILE ||
       theS->PSpulse == PARASITIC_PULSES_ID_ROOTFILE) &&
      theS->PulseIntensity > DEDICATED_PULSES_INTMIN &&
      theS->PulseIntensity < DEDICATED_PULSES_INTMAX) {
    // Allow high intensity parasitic pulses to be treated as regular dedicated
    // pulses
...
void CalculateDeadTimeHisto(TH1D* h1, double TOFD, double DeadTime_ns) {
  int nbins = h1->GetNbinsX();
  double val, err, ene1, ene2, tof;
  for (int i = 1; i <= nbins; i++) {
    val = h1->GetBinContent(i);
    ene1 = h1->GetBinLowEdge(i);
    ene2 = h1->GetBinLowEdge(i + 1);
    tof = DeltaEnergyToTime(ene1, ene2, TOFD);  // time in ns
    h1->SetBinContent(i, 1 / (1 - (val * DeadTime_ns / tof)));
  }
}
