...
TH1D *GetMonHistogram(const char *hname, int RunNumber,
                      string FnameBase = "Monitoring01");
TH1D *GetMonHistogram(const char *hname, int NRuns, int *RunList,
                      string FnameBase = "Monitoring01");
TH1D *GetMonHistogram(const char *hname, string RunType,
                      string FnameBase = "Monitoring01");
TGraph *GetResiduals(TH1D *h1, TH1D *h2, double Xmin,
                     double Xmax); // h1-> exp, h2->MC
void ReCalculateUncertainties(TH1D *h1);
void UpdateMinimumAndMaximum(TH1D *h1, double xmin, double xmax, double &MinVal,
                             double &MaxVal);
void UpdateMinimumAndMaximum(TGraph *gr1, double xmin, double xmax,
                             double &MinVal, double &MaxVal);
double IntegrateHisto(TH1D *h1, double xmin, double xmax, double &err,
                      bool DONOTMULTBYX, int CORRELATEDERRORS);
TH1D *ShiftHisto(TH1D *h1, double shift); // Shift h1 by a factor "shift"
TH1D *GetIntegratedHisto(TH1D *h1, int nbins, double Xmin, double Xmax,
                         bool IsLog, bool DONOTMULTBYX = false,
                         int CORRELATEDERRORS = false,
                         bool DivideByBinWidth = false);
TH1D *GetIntegratedHisto(TH1D *h1, TH1D *h2_base, bool DONOTMULTBYX,
                         int CORRELATEDERRORS, bool DivideByBinWidth);
TH1D *GetIntegratedHisto(TGraph *gr1, int nbins, double Xmin, double Xmax,
                         bool IsLog);
int GetEnIntEmaxEmin01(const char *fname, int EnInt, double &Emin,
                       double &Emax);
int GetEnIntEmaxEmin(int RunNumber, int EnInt, double &Emin, double &Emax,
                     string FnameBase = "Monitoring01");
int GetEnIntEmaxEmin(string RunType, int EnInt, double &Emin, double &Emax,
                     string FnameBase = "Monitoring01");
int GetEnTOFCuts(const char *fnamebase, double &Emin, double &Emax);
TH1D *MakeRatio(TH1D *h1, TH1D *h2, int Opt);
double TakeBestShiftFactor(TH1D *h1, TH1D *h2, double Xmin, double Xmax,
                           double ShiftMin, double ShiftMax, int NSteps,
                           bool NORM = true, int HalfNPToFitParabolic = 25);
double TakeBestShiftFactor(TH1D *h1, TH1D *h2, double Xmin, double Xmax,
                           TH1D *hComp, bool NORM = true,
                           int HalfNPToFitParabolic = 25);
double GetChi2(TH1D *h1, TH1D *h2, double Xmin, double Xmax, bool NORM = true);
//====================================================================================

/*
//Transforma el típico histograma con fluctuaciones en uno sin
void MakeNoBeam(TH1D* h1,double EnMin=0.02){

  double BinMin=h1->FindBin(EnMin);
  int nbins=h1->GetNbinsX();
  double err=0;
  double integral=h1->IntegralAndError(BinMin,nbins,err);
...
// Returns the best shift-factor so that h2 transformed with shift-factor
// mathches h1 between Xmin and Xmax
double TakeBestShiftFactor(TH1D *h1, TH1D *h2, double Xmin, double Xmax,
                           double ShiftMin, double ShiftMax, int NSteps,
                           bool NORM, int HalfNPToFitParabolic) {
  TH1D *hComp = new TH1D("hComp", "hComp", NSteps, ShiftMin, ShiftMax);
  double ShifFactor = TakeBestShiftFactor(h1, h2, Xmin, Xmax, hComp, NORM,
                                          HalfNPToFitParabolic);
  delete hComp;
  return ShifFactor;
}
...
// mathches h1 between Xmin and Xmax
double TakeBestShiftFactor(TH1D *h1, TH1D *h2, double Xmin, double Xmax,
                           TH1D *hComp, bool NORM, int HalfNPToFitParabolic) {
  ...
  if (minbin < 2 || minbin > NSteps - 1) {
    cout << " ########## WARNING calculating best ShiftFactor: minbin is "
         << minbin << " ##########" << endl;
    delete hComp;
    return -1;
  }
  ...
  // int HalfNPToFitParabolic=25;
  int fitstart = minbin - HalfNPToFitParabolic;
  if (fitstart < 1) {
    fitstart = 1;
  }
  int fitend = minbin + HalfNPToFitParabolic;
  if (fitend > NSteps) {
    fitend = NSteps;
  }
  ...
  cout << "min(chi2)= "
       << a * (result + trval) * (result + trval) + b * (result + trval) + c
       << " at MF= " << result << " --> shift = " << 1. / result << endl;
  //--------------------------------------------------------
...
TH1D *MakeRatio(TH1D *h1, TH1D *h2, int Opt) {
  ...
  for (int i = 1; i <= nbins; i++) {
    ...
    if (Opt == 1) {
      ...
      val2 = IntegrateHisto(h2, h1->GetBinLowEdge(i), h1->GetBinLowEdge(i + 1),
                            dum, false, 0) /
             (h1->GetBinLowEdge(i + 1) - h1->GetBinLowEdge(i));
    }
    if (val2 > 0) {
...
int GetEnIntEmaxEmin(int RunNumber, int EnInt, double &Emin, double &Emax,
  ...
  Emax = 0;
  char fname[300];
  sprintf(fname, "%s/%s_%d.txt", FILES_OUT_DIRNAME, FnameBase.c_str(),
          RunNumber);
...
int GetEnIntEmaxEmin(string RunType, int EnInt, double &Emin, double &Emax,
  ...
  int NEneIntervals =
      GetEnIntEmaxEmin(RunList[0], EnInt, Emin, Emax, FnameBase);

  return NEneIntervals;
}

TH1D *GetIntegratedHisto(TH1D *h1, int nbins, double Xmin, double Xmax,
                         bool IsLog, bool DONOTMULTBYX, int CORRELATEDERRORS,
                         bool DivideByBinWidth) {
  double *xlims = new double[nbins + 1];
  for (int i = 0; i <= nbins; i++) {
    if (IsLog) {
      xlims[i] = Xmin * pow(Xmax / Xmin, i / (double)(nbins));
    } else {
      xlims[i] = Xmin + i / (double)(nbins) * (Xmax - Xmin);
    }
  }
...
TH1D *ShiftHisto(TH1D *h1, double shift) {
  ...
  for (int i = 1; i <= nbins; i++) {
    ...
    IntegEmin = emin / shift;
    if (IntegEmin < LowLim) {
      IntegEmin = LowLim;
    }
    IntegEmax = emax / shift;
    if (IntegEmax > HighLim) {
      IntegEmax = HighLim;
    }
...
// CORRELATEDERRORS=0,-1,1 --> no correlated,correlated +1 when positive and -1
// when negative, correlated +1
double IntegrateHisto(TH1D *h1, double xmin, double xmax, double &err,
                      bool DONOTMULTBYX, int CORRELATEDERRORS) {
  int nb = h1->GetNbinsX();
  double Sum = 0;
  ...
  for (int i = 1; i <= nb; i++) {
    ...
    if (fraction > 0) {
      if (DONOTMULTBYX) {
        Sum += val * fraction;
        //
        if (CORRELATEDERRORS == -1) {
          if (val > 0) {
            SumErr += valerr * fraction;
          } else {
            SumErr -= valerr * fraction;
          }
        } else if (CORRELATEDERRORS == +1) {
          SumErr += valerr * fraction;
        }
        //
        ...
        if (CORRELATEDERRORS == -1) {
          if (val > 0) {
            SumErr += valerr * (maxval - minval) * fraction;
          } else {
            SumErr -= valerr * (maxval - minval) * fraction;
          }
        } else if (CORRELATEDERRORS == +1) {
        ...
        //
        SumErr2 +=
            valerr * (maxval - minval) * valerr * (maxval - minval) * fraction;
      }
    }
  }
...
void UpdateMinimumAndMaximum(TGraph *gr1, double xmin, double xmax,
  ...
  for (int i = 0; i < np; i++) {
    ...
    if (xval > xmin && xval < xmax && yval < MinVal) {
      MinVal = yval;
    }
    if (xval > xmin && xval < xmax && yval > MaxVal) {
      MaxVal = yval;
    }
  }
}

// Calcula de nuevo los errores como la raíz cuadrada del contenido de los
// bines:
void ReCalculateUncertainties(TH1D *h1) {
  int nbins = h1->GetNbinsX();
...
TGraph *GetResiduals(TH1D *h1, TH1D *h2, double Xmin, double Xmax) {
  ...
  for (int i = 1; i <= nbins; i++) {
    ...
    if ((xval > Xmin || Xmin < 0) && (xval < Xmax || Xmax < 0) &&
        h1->GetBinError(i) > 0) {
      residual =
          (h1->GetBinContent(i) - h2->Interpolate(xval)) / h1->GetBinError(i);
      xvals[np] = xval;
      yvals[np] = residual;
      np++;
    }
...
TH1D *GetMonHistogram(const char *hname, int RunNumber, string FnameBase) {
  char fname[300];
  sprintf(fname, "%s/%s_%d.root", FILES_OUT_DIRNAME, FnameBase.c_str(),
          RunNumber);
...
TH1D *GetMonHistogram(const char *hname, const char *fname) {
  ...
  if (!h1) {
    cout << " ############# WARNING: histogram " << hname
         << " does not exist in " << fname << " #############" << endl;
    return 0;
  }
  h1->Sumw2();
  h1->SetDirectory(0);
