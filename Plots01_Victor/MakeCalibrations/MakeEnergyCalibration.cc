...
void CalibrateWithChi(string MCfname, string Expfname, string outFolder, int DetN, string Source, double GammaEnergy, int rebin, double Percen,
                      int npRes, double Res, double PERRes, int npCalib, double Calib, double PERCalib);

void CreateExpfname(string DataFolder, string RunList, string FolderRootFiles, string Expfname, int DetectorsCalibrated, string SourceType,
                    int NumberBinsExpfname, double EMaxExpfname);
void CreateExpfnameRemoveBackground(string DataFolder, string RunList, string FolderRootFiles, string Expfname, string ExpfnameBackground,
                                    int DetectorsCalibrated, string SourceType, int NumberBinsExpfname, double EMaxExpfname);
void MakeEnergyCalibration(string DataFolder, string RunList, string FolderRootFiles, string Expfname, string ExpfnameBackground,
                           int DetectorsCalibrated, string SourceType, string Source, string SourceTypeBakground, int NumberBinsExpfname,
                           double EMaxExpfname, string MCfname, string outFolder, double GammaEnergy, int rebin, double Percen, int npRes, double Res,
                           double PERRes, int npCalib, double Calib, double PERCalib);
...
int main(int argc, char **argv) {
  ...
  if (argc == 3) {
    ...
    MakeEnergyCalibration(DataFolder, RunList, FolderRootFiles, Expfname, ExpfnameBackground, DetectorsCalibrated, SourceType, Source,
                          SourceTypeBakground, NumberBinsExpfname, EMaxExpfname, MCfname, outFolder, GammaEnergy, rebin, Percen, npRes, Res, PERRes,
                          npCalib, Calib, PERCalib);
  } else {
    cout << " ******* Input has to be like 22 or 2 and has  " << argc - 1 << "  *************** entries=  " << endl;
    exit(1);
  }
}
...
void MakeEnergyCalibration(string DataFolder, string RunList, string FolderRootFiles, string Expfname, string ExpfnameBackground,
  ...
  if (f->IsZombie()) {
    cout << " ######   file " << ExpfnameBackground.c_str() << " does not exist we will created #######" << endl;
    CreateExpfname(DataFolder, RunList, FolderRootFiles, ExpfnameBackground, DetectorsCalibrated, SourceTypeBakground, NumberBinsExpfname,
                   EMaxExpfname);
  }

  f = new TFile(Expfname.c_str(), "READ");
  if (f->IsZombie()) {
    cout << " ######   file " << Expfname.c_str() << " does not exist we will created #######" << endl;

    CreateExpfnameRemoveBackground(DataFolder, RunList, FolderRootFiles, Expfname, ExpfnameBackground, DetectorsCalibrated, SourceType,
                                   NumberBinsExpfname, EMaxExpfname);
  }
  CalibrateWithChi(MCfname, Expfname, outFolder, DetectorsCalibrated, Source, GammaEnergy, rebin, Percen, npRes, Res, PERRes, npCalib, Calib,
                   PERCalib);
}
...
void CreateExpfnameRemoveBackground(string DataFolder, string RunList, string FolderRootFiles, string Expfname, string ExpfnameBackground,
  ...
  int nRuns = TakeRunList(RunList.c_str(), SourceType, FolderRootFiles.c_str(), SRunList);
  ...
  for (int i = 0; i < nRuns; i++) {
    cout << " open " << SRunList[i].c_str() << " Detname " << DetectorsCalibrated << " detN " << DetectorsCalibrated << endl;
    ifstream in(SRunList[i].c_str());
    in.close();
    if (!in.good()) {
      cout << " ######  WARNING, file " << SRunList[i] << " does not exist #######" << endl;
    } else {
  ...
  TFile *f = new TFile(ExpfnameBackground.c_str(), "READ");
  if (f->IsZombie()) {
    cout << " ######   file " << ExpfnameBackground.c_str() << " does not exist  #######" << endl;
    exit(1);
  }
...
void CalibrateWithChi(string MCfname, string Expfname, string outFolder, int detN, string Source, double GammaEnergy, int rebin, double Percen,
                      int NRESOLUTIONS, double Res, double PERRes, int NCALIBENE, double Calib, double PERCalib) {
  cout << "starting" << endl;
  time_t starttime = time(0);
  int EnergyAdjustCalibrationinKeV = 0;  // it is a variable used to add a small
                                         // shift in the energy calibration
  int const MAXNGAMMAS = 100000000;      // maximo numero de gammas a coger del Monte Carlo
  ...
  cout << "    " << NRESOLUTIONS << " points for resolution between " << ResMin << " and " << ResMax << endl;
  cout << "    " << NCALIBENE << " points for calibration between " << CalibMin << " and " << CalibMax << endl;
  ...
  double ComptonEdgeEnergy = GammaEnergy * (1 - 1 / (1 + 2 * GammaEnergy / 0.511));
  ...
  TH2D *h2 = new TH2D("h2", "", NRESOLUTIONS, ResMin - DeltaRes, ResMax + DeltaRes, NCALIBENE, CalibMin - DeltaCalib, CalibMax + DeltaCalib);
  ...
  for (int i = 1; i <= NRESOLUTIONS; i++) {
    for (int j = 1; j <= NCALIBENE; j++) {
      ...
      FillTH1DFromMC(h1, MCfname.c_str(), detN, CalibFactor, a_factor, EnergyAdjustCalibrationinKeV);
      // cout << "CalibFactor " << CalibFactor << " "      <<
      // "ResAtComptonEdge " << ResAtComptonEdge << " "       << "Percen  " <<
      // Percen << " " << "a_factor  " << a_factor << endl << endl;
      Chi2 = CompareHistograms(hExp, h1, ComptonEdgeEnergy * (100 - Percen) / (100 * CalibFactor),
  ...
  int firstbin = hExp->FindBin(ComptonEdgeEnergy * (100 - Percen) / 100 / CalibFactor);
  int lastbin = hExp->FindBin(ComptonEdgeEnergy * (100 + Percen) / 100 / CalibFactor);
  ...
  cout << "firstbin" << firstbin << "lastbin" << lastbin << "area1 " << area1 << " area2 " << area2 << " area3 " << area3 << endl;
  ...
  hExp->GetYaxis()->SetRangeUser(0, 2.0 * h1->GetBinContent(h1->FindBin(0.8 * ComptonEdgeEnergy / CalibFactor)));
  cout << "Channel used to plot " << 0.8 * ComptonEdgeEnergy / CalibFactor << endl;
  ...
  sprintf(rootoutfname, "%s/Calibration_Det%d_%s", outFolder.c_str(), detN, Source.c_str());
  ...
  cout << "Alpha parameter  " << (ResAtComptonEdge * sqrt(ComptonEdgeEnergy) / 2.35) * (ResAtComptonEdge * sqrt(ComptonEdgeEnergy) / 2.35) << "  "
       << endl;
  ...
  outdata << ComptonEdgeEnergy << "  " << ResFinal << "  " << CalibFactor << endl;
  outdata.close();
}
//////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////
...
void FitEnergy(string outfolder, int detn) {
  ...
  out << endl << endl << "********************NameFile************** " << endl << OutNameDet << endl;
  double maximumAmp = 0;
  ...
  TF1 *myfitpol1 = new TF1("myfitpol1", "[0] + [1]*x", 0.1, 100000);  // pol1
  ...
  TF1 *myfitpol2 = new TF1("myfitpol2", "[0] + [1]*x + [2]*x*x", 0.1, 100000);  // pol2
  myfitpol2->SetParLimits(0, 0.00, 0.02);
  ...
  TF1 *myfitpol3 = new TF1("myfitpol3", FunPol2andPol1, 0.1, 100000, 4);  // pol3
  myfitpol3->SetParLimits(0, 0.000, 0.02);
  myfitpol3->SetParLimits(3, 0.5 / p1pol1, 0.5 / p1pol1);
  myfitpol3->SetParLimits(3, 0, 0);
  myfitpol3->SetRange(0.1, 1.1 / p1pol1);

  cout << "Limits used:" << endl;
  cout << "Par 0 " << 0.000 << " to " << 0.01 << endl;
  cout << "Par 3 " << 0.5 / p1pol1 << " to " << 0.5 / p1pol1 << endl;
  cout << "Range  " << 0.1 << " to " << 1.1 / p1pol1 << endl;

  // First we adjusted only the low energy . In particular we adjusted below 1.1 MeV
  gr->Fit("myfitpol3", "R0");
  ...
  cout << "p0pol12 " << p0pol12 << " p1pol12 " << p1pol12 << " p2pol12 " << p2pol12 << " p3pol12 " << p3pol12 << endl;

  // Second we adjusted all the range with some limits from the previous fit
  myfitpol3->SetParLimits(0, 0.000, 0.02);
  myfitpol3->SetParLimits(1, p1pol12 * 0.95, p1pol12 * 1.051);
  myfitpol3->SetParLimits(2, p2pol12 * 0.01, p2pol12 * 100);
  myfitpol3->SetParLimits(3, 0.5 / p1pol1, 2.5 / p1pol1);
  myfitpol3->SetRange(0.1, 20.0 / p1pol1);

  cout << "Limits used:" << endl;
  cout << "Par 0 " << 0.000 << " to " << 0.01 << endl;
  cout << "Par 1 " << p1pol12 * 0.95 << " to " << p1pol12 * 1.051 << endl;
  cout << "Par 3 " << 0.5 / p1pol1 << " to " << 0.5 / p1pol1 << endl;
  cout << "Range  " << 0.1 << " to " << 20.0 / p1pol1 << endl;
  gr->Fit("myfitpol3", "R0");
  ...
  for (int j = 0; j < np; j++) {
    ...
    SSresLin += (p0pol1 + Amp[j] * p1pol1 - E[j]) * (p0pol1 + Amp[j] * p1pol1 - E[j]);

    SSresPol += (p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2 - E[j]) * (p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2 - E[j]);
    SSresPol12 += (myfitpol3->Eval(Amp[j]) - E[j]) * (myfitpol3->Eval(Amp[j]) - E[j]);
  }
  ...
  cout << "SSresLin= " << SSresLin << " SStotLin= " << SStot << " R2Lin " << R2Lin << endl;
  cout << "SSresPol= " << SSresPol << " SStotLin= " << SStot << " R2Pol " << R2Pol << endl;
  cout << "SSresPol12= " << SSresPol12 << " SStotLin= " << SStot << " R2Pol " << R2Pol12 << endl;
  ...
  for (int j = 0; j < np; j++) {
    ...
    PointCheckParabola[j] = (p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2) / E[j];
    PointCheckPol12[j] = myfitpol3->Eval(Amp[j]) / E[j];
    cout << "TEST " << j << " " << E[j] << " " << (p0pol1 + Amp[j] * p1pol1) / E[j] << endl;

    cout << "Pol1 energy Real E " << E[j] << " using " << p0pol1 + Amp[j] * p1pol1 << " Diferance % "
         << 100 * (p0pol1 + Amp[j] * p1pol1 - E[j]) / E[j] << endl;
    cout << "Pol2 energy Real E " << E[j] << " using " << p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2 << " Diferance % "
         << 100 * (p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2 - E[j]) / E[j] << endl;

    cout << "Pol12 energy Real E " << E[j] << " using " << myfitpol3->Eval(Amp[j]) << " Diferance % " << 100 * (myfitpol3->Eval(Amp[j]) - E[j]) / E[j]
         << endl;
    cout << endl << endl;

    out << "Pol1 energy Real E " << E[j] << " using " << p0pol1 + Amp[j] * p1pol1 << " Diferance % " << 100 * (p0pol1 + Amp[j] * p1pol1 - E[j]) / E[j]
        << endl;
    out << "Pol2 energy Real E " << E[j] << " using " << p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2 << " Diferance % "
        << 100 * (p0pol2 + Amp[j] * p1pol2 + Amp[j] * Amp[j] * p2pol2 - E[j]) / E[j] << endl;
    out << "Pol12 energy Real E " << E[j] << " using " << myfitpol3->Eval(Amp[j]) << " Diferance % " << 100 * (myfitpol3->Eval(Amp[j]) - E[j]) / E[j]
        << endl;
    cout << endl << endl;
    out << endl << endl;
  }
  ...
  h3->GetYaxis()->SetRangeUser(0.95, 1.05);
  h3->Draw();
  l2->Draw();
  l3->Draw();
  ...
  cout << "SecondDegree" << endl << p0pol2 << "  " << p1pol2 << " " << p2pol2 << endl;
  cout << " 0 0 0 0  " << p0pol2 << "  " << p1pol2 << " " << p2pol2 << endl << endl;

  cout << "Pol12" << endl << p0pol12 << "  " << p1pol12 << " " << p2pol12 << " " << p3pol12 << endl;
  cout << p0pol12 << "  " << p1pol12 << " " << p2pol12 << " " << p3pol12 << " " << -p2pol12 * p3pol12 * p3pol12 + p0pol12 << " "
       << 2 * p2pol12 * p3pol12 + p1pol12 << "  0  " << endl
       << endl;

  out << "FisrtDegree" << endl << p0pol1 << "  " << p1pol1 << endl;
  out << " 0 0 0 0   " << p0pol1 << "  " << p1pol1 << " 0  " << endl << endl;

  out << "SecondDegree" << endl << p0pol2 << "  " << p1pol2 << " " << p2pol2 << endl;
  out << " 0 0 0 0  " << p0pol2 << "  " << p1pol2 << " " << p2pol2 << endl << endl;

  out << "Pol12" << endl << p0pol12 << "  " << p1pol12 << " " << p2pol12 << " " << p3pol12 << endl;
  out << p0pol12 << "  " << p1pol12 << " " << p2pol12 << " " << p3pol12 << " " << -p2pol12 * p3pol12 * p3pol12 + p0pol12 << " "
      << 2 * p2pol12 * p3pol12 + p1pol12 << "  0  " << endl
      << endl;
  ...
  // p2pol2  << endl;
  outAllDet << detn << " " << p0pol12 << "  " << p1pol12 << " " << p2pol12 << " " << p3pol12 << " " << -p2pol12 * p3pol12 * p3pol12 + p0pol12 << " "
            << 2 * p2pol12 * p3pol12 + p1pol12 << "  0  " << endl;
  outAllDet.close();
...
void FitRes(string outfolder, int detn) {
  ...
  TH2D *h2 = new TH2D("", "Energy resolution", 1000, 0, E[np - 1] * 50, 1000, 0, 0.6);
  TPad *pad1;
  TPad *pad2;
  pad1 = new TPad("pad1", "Pad", 0.0, 0.33, 1., 1., 10);
  pad2 = new TPad("pad2", "Pad", 0.0, 0.02, 1., 0.33, 10);
  ...
  for (int j = 0; j < np; j++) {
    ...
    cout << "energy Real E " << E[j] << " Res " << Res[j] << " using resfun " << ResFunNormal(E[j], par0v, par1v) << " Diferance % "
         << 100 * (ResFunNormal(E[j], par0v, par1v) - Res[j]) / Res[j] << endl;
    out << "energy Real E " << E[j] << " Res " << Res[j] << " using resfun " << ResFunNormal(E[j], par0v, par1v) << " Diferance % "
        << 100 * (ResFunNormal(E[j], par0v, par1v) - Res[j]) / Res[j] << endl;
  }
  h2->GetXaxis()->SetTitleSize(0.090);
  h2->GetXaxis()->SetLabelSize(0.090);
