...
int main(int argc, char** argv) {
  ...
  // sprintf(DirNameoutMerged, "%s",
  //         "/eos/home-v/valcayne/nTOFDataProcessing/2026_Sm/2DHistos/"
  //         "v02Merge");

  // sprintf(outdirGainCorrected, "%s",
  //         "/eos/home-v/valcayne/nTOFDataProcessing/2026_Sm/2DHistos/"
  //         "v02");
  ...
  char outfn_root[400];
  char outfn_txt[400];
  sprintf(outfn_root, "%s/%s_%s.root", DirNameoutMerged, StartFname,
          RunType.c_str());
  sprintf(outfn_txt, "%s/%s.txt", DirNameoutMerged, RunType.c_str());
  ...
  for (int i = 0; i < nRuns; i++) {
    sprintf(fname, "%s/%s_%d.root", outdirGainCorrected, StartFname,
            RunList[i]);
    if (GetFileSize(fname) < 1000) {
      ...
      if (!txtdone) {
        ...
        sprintf(tmpcpmmand, "cp %s/%s_%d.txt %s", outdirGainCorrected,
                StartFname, RunList[i], outfn_txt);
